# strategies/rsi2_above_90_strategy.py
...
def generate_signals(df) -> pd.DataFrame:
    """
    2-Period RSI > 90 Short Strategy:
      1) ETF closes under its 200-day MA
      2) The 2-period RSI closes above 90 -> Short on the close
      3) If the 2-period RSI closes above 94, short another unit
      4) Exit when the ETF closes under its 5-period MA or above 200-day MA
    ...
    for i in range(1, len(df)):
        ...
        # Add unit condition (only if we're already in position)
        if in_position and rsi_above_94.iloc[i] and df['Units'].iat[i] == 1:
            # Add another unit
            df['Signal'].iat[i] = -1
            df['Units'].iat[i] = 2
